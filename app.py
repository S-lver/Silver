...
# ===== Initialize AI Provider (Replaces Groq client) =====
ai_provider = AIProvider()

# ===== Model Configuration (Keep for reference) =====
MODEL_CONFIG = {
...
# ===== Select Model Based on Complexity (Keep for reference) =====
def select_model(messages):
    """Choose the right model based on conversation complexity"""
    # This is now handled by the AIProvider, but keep for reference
    total_chars = sum(
        len(msg.get('content', '')) 
        for msg in messages 
...
- You can use emojis sparingly to add personality to responses'''
        ...
        # Select model (AIProvider handles this internally, but we'll use it for logging)
        model_choice = select_model(optimized_messages)
        ...
        # Log the request
        logger.info(f"Request {request_id}: Model={model_choice}, IP={client_ip}, Messages={len(optimized_messages)}, EstTokens={estimated_tokens}")
        
        # Get response from AI Provider (with streaming)
